...
def heuristByColsRANDOM(pesos,uRows,pCols,dictCols):
    ColumnWeight = np.zeros((len(pCols),2))
    #print('pcols',len(pCols))
    for i in range(0,len(pCols)):
        lRows = dictCols[pCols[i]]
        ColumnWeight[i,0] = pCols[i]
        ColumnWeight[i,1] = float(pesos[pCols[i]])/len(list(set(lRows).intersection(set(uRows)))) #Revisar*** Es costoso
    ColumnWeight = ColumnWeight[ColumnWeight[:,1].argsort()]
    #Option1 = np.random.randint(0,2)  #ACORTO EL RANGO PARA EMPEORAR LA HEURISTICA
    Option1 = 1
    if Option1 == 0:
        column = int(ColumnWeight[np.random.randint(1,len(pCols)),0])
    else:
        #column = int(ColumnWeight[np.random.randint(0,2),0])
        column = int(ColumnWeight[0,0])

    return column
def heuristByColsRANDOM2(pesos,uRows,pCols,dictCols):
    ColumnWeight = np.zeros((len(pCols),2))
    #print('pcols',len(pCols))
    for i in range(0,len(pCols)):
        lRows = dictCols[pCols[i]]
